...
with requested_scoring_dates(scoring_date) as (
    ...
    {% if scoring_dates is not none %}
        ...
        from (
        {% for scoring_date in scoring_dates %}
            select date '{{ scoring_date }}' as scoring_date{% if not loop.last %} union all{% endif %}
        {% endfor %}
    ...
    {% else %}
        ...
        {{ portable_month_grid(this.schema ~ '.event_version_batches') }}
    {% endif %}
    ...
    join {{ ref('events_union') }} events
     ...
     and events.event_date >= ({{ dbt.dateadd('month', -6, 'grid.scoring_date') }})::date
    ...
    select
        ...
        count(case when events.event_type = 'inspection'
              and events.event_date >=
                  ({{ dbt.dateadd('month', -6, 'carriers.scoring_date') }})::date then 1 end)::bigint as inspections_6m,
        coalesce(sum(case when events.event_type = 'inspection'
              and events.event_date >=
                  ({{ dbt.dateadd('month', -6, 'carriers.scoring_date') }})::date then events.violation_count end), 0)::bigint as violations_6m,
        coalesce(sum(case when events.event_type = 'inspection'
              and events.event_date >=
                  ({{ dbt.dateadd('month', -6, 'carriers.scoring_date') }})::date then events.oos_violation_count end), 0)::bigint as oos_violations_6m,
        count(distinct case when events.event_type = 'crash'
              and events.event_date >=
                  ({{ dbt.dateadd('month', -24, 'carriers.scoring_date') }})::date then events.carrier_crash_key end)::bigint as crashes_24m,
        max(case when events.event_type = 'inspection'
              and events.event_date >=
                  ({{ dbt.dateadd('month', -6, 'carriers.scoring_date') }})::date then events.event_date end) as last_inspection_date
    ...
    left join {{ ref('events_union') }} events
     ...
     and events.event_date >=
         ({{ dbt.dateadd('month', -24, 'carriers.scoring_date') }})::date
...
select
    ...
    oos_violations_6m,
    crashes_24m,
    {{ portable_ratio('violations_6m', 'inspections_6m') }}
        as violations_per_inspection,
    {{ portable_ratio('oos_violations_6m', 'violations_6m') }}
        as oos_violation_rate,
    {{ dbt.datediff('last_inspection_date', 'scoring_date', 'day') }}::integer as days_since_last_inspection,
