...
from ml.source_records import CRASH_COLUMNS, parse_crash_report
...
def _scan_snapshot(
    ...
    with (
        ...
        for batch in source.iter_batches(batch_size=65536, columns=CRASH_COLUMNS):
            ...
            for row in selected.to_pylist():
                if not row["DOT_NUMBER"].strip():
                    ...
                    continue
                try:
                    reports.append(parse_crash_report(row))
